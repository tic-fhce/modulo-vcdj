...
public class WorkflowService {
    ...
    public Map<String, String> inicioFlujo(User user, InicioRequest request) {
        ...
        workflowRepository.crearSeguimiento(nroTramite, flujo, "P1", rolId, comentario, unidadId, user.getId(), fechaHoraFutura);

        Map<String, String> responseBody = new HashMap<>();
        responseBody.put("message", "Inicio Flujo Correcto");
    ...
    public String siguienteFormulario(User user, SiguienteFormRequest request) {
        ...
        if (rolId.equals(1) || rolId.equals(2) || rolId.equals(13)) {
            ...
            siguienteUserId = workflowRepository.getUserIdByRoleIdAndCarrera(rolId, carrera);
        }
        ...
        if (partes.length > 1) {
            ...
            workflowRepository.crearSeguimientoFin(tramiteId, flujo, x, rolId, comentario, carrera, siguienteUserId, fechaHoraFutura);

            workflowRepository.updateTramiteEstadoTerminado(tramiteId, flujo);

            return "Finalizado";
        } else {
            workflowRepository.crearSeguimiento(tramiteId, flujo, procesoSiguiente, rolId, comentario, carrera, siguienteUserId, fechaHoraFutura);
